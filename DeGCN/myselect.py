...
class EnsembleEvaluator:
    ...
    def evaluate(self, alpha: np.ndarray) -> Tuple[float, float, np.ndarray]:
        ...
        if acc > self.best_acc:
            self.best_acc = acc
            np.save('predA.npy', np.array(fused_results))
    ...
    def optimize_weights_bayesian(self, init_points=5, n_iter=50):
        ...
        pbounds = {f'alpha_{i}': (0.0, 1.0) for i in range(self.num_modalities)}
...
def main():
    modality_configs = [
        ModalityConfig("de_J_Score", "work_dir/val/DeGCN_joint/weights"),
        ModalityConfig("Te_Score", "TeGCN/work_dir/val/tegcn_jb"),
        ModalityConfig("stt_J_Score", "sttformer/work_dir/val/joint"),
        ModalityConfig("stt_JM_Score", "sttformer/work_dir/val/motion"),
        ModalityConfig("de_JB_Score", "work_dir/val/DeGCN_jbf/weights"),
        ModalityConfig("de_A_Score", "work_dir/val/DeGCN_angle/weights"),
        ModalityConfig("ske_J_Score", "SkateFormer-main/work_dir/val/ske_joint"),
        ModalityConfig("ske_B_Score", "SkateFormer-main/work_dir/val/ske_bone"),
        ModalityConfig("de_JMf_Score", "work_dir/val/DeGCN_jmf/weights"),
    ]
